...
class AnnotationAgreement:
    ...
    def compute_kappa(self):
        ...
        self.data.ix[:, 4] = self.data.ix[:, 4].replace(1, 2)
        self.data.ix[:, 7] = self.data.ix[:, 7].replace(1, 2)

        kappa = cohen_kappa_score(self.data.ix[:, 4], self.data.ix[:, 7])
        #anno_l = self.data.ix[:, [4,7]].to_numpy().tolist()
        #kappa = krippendorff.alpha(anno_l)
        return kappa
